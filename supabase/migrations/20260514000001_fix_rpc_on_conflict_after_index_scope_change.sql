-- Migration: 20260514000001_fix_rpc_on_conflict_after_index_scope_change.sql
-- Description: 20260512000006_fix_kaynak_unique_scope.sql partial unique index'in
-- predicate'ini daralttı:
--   ESKİ uq_cari_hareketler_kaynak             : WHERE kaynak_id IS NOT NULL AND kaynak_tipi IS NOT NULL
--   YENİ uq_cari_hareketler_kaynak_tahakkuk    : WHERE kaynak_id IS NOT NULL
--                                                AND kaynak_tipi IN ('aidat_kayit','gecikme_faizi','fatura')
--
-- Ancak üç RPC (fn_bulk_charge_interest, fn_create_fatura_atomic, fn_update_fatura_atomic)
-- hâlâ eski/geniş `ON CONFLICT (...) WHERE kaynak_id IS NOT NULL AND kaynak_tipi IS NOT NULL`
-- inference predicate'ini kullanıyordu. PostgreSQL ON CONFLICT inference kuralı: kullanıcı
-- predicate'i, index predicate'ini İMPLY etmelidir. `kaynak_tipi IS NOT NULL`,
-- `kaynak_tipi IN (...)` ifadesini imply etmediği için PG hata fırlatır:
--   42P10: there is no unique or exclusion constraint matching the ON CONFLICT specification
--
-- errorHandler bu kodu mapping etmediği için kullanıcıya yansıyan mesaj:
--   500 { "success": false, "error": "Veritabanı hatası oluştu" }
--
-- Tekrar üretim: Üye Detay → "Faiz Borç İşle" → "Faizleri Borçlandır".
-- Aynı hata, fatura kayıt/güncellemede gelen fatura (firma + proje) durumunda da çıkar.
--
-- Fix: Üç RPC'nin ON CONFLICT WHERE clause'ı yeni partial index predicate'i ile bire bir
-- eşleşecek şekilde güncellendi. RPC gövdesinin diğer kısımları (audit_actor pattern,
-- iş mantığı) 20260511000003 sürümünden değiştirilmeden kopyalandı.

BEGIN;

-- =====================================================================
-- 1. fn_bulk_charge_interest — ON CONFLICT WHERE clause fix
-- =====================================================================
...
COMMENT ON FUNCTION public.fn_bulk_charge_interest(UUID[], UUID) IS
  'Coklu aidat icin faiz tahakkuku. ON CONFLICT predicate uq_cari_hareketler_kaynak_tahakkuk'
  ' (20260512000006) ile bire bir eslesir. p_actor_id verilirse app.actor_id session var set edilir.';

-- =====================================================================
-- 2. fn_create_fatura_atomic — ON CONFLICT WHERE clause fix
-- =====================================================================
...
COMMENT ON FUNCTION public.fn_create_fatura_atomic(JSONB, JSONB, UUID) IS
  'Fatura + kalemler + (gelen ise) cari hareket atomik. ON CONFLICT predicate'
  ' uq_cari_hareketler_kaynak_tahakkuk (20260512000006) ile bire bir eslesir.'
  ' p_actor_id verilirse app.actor_id session var set edilir.';

-- =====================================================================
-- 3. fn_update_fatura_atomic — ON CONFLICT WHERE clause fix
-- =====================================================================
...
COMMENT ON FUNCTION public.fn_update_fatura_atomic(UUID, JSONB, JSONB, UUID) IS
  'Fatura + kalemler + cari hareket guncelleme atomik. ON CONFLICT predicate'
  ' uq_cari_hareketler_kaynak_tahakkuk (20260512000006) ile bire bir eslesir.'
  ' p_actor_id verilirse app.actor_id session var set edilir.';
