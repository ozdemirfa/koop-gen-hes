-- Migration: 20260426000004_fix_aidat_view_aliases.sql
-- Description: Add backward compatibility aliases to aidat_detaylari view.
...
SELECT 
    ...
    (at.katsayi_tutari * COALESCE(s.serefiye_orani, 1.00)) as hesaplanan_tutar, -- Alias for backward compatibility
    
    -- Cari Hareketlerden Gelen Gerçek Veriler
    COALESCE(ct.total_accrued, 0) as toplam_tahakkuk,
    COALESCE(ct.total_accrued, 0) as toplam_borc, -- Alias for backward compatibility
    
    COALESCE(ct.total_paid, 0) as toplam_odenen,
    COALESCE(ct.total_paid, 0) as dinamik_odenen_tutar, -- Alias for backward compatibility
    
    COALESCE(ct.total_interest, 0) as toplam_faiz,
    COALESCE(a.gecikme_faizi, 0) as gecikme_faizi, -- Use table value or calculated
