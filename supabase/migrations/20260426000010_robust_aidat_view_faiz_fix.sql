-- Migration: 20260426000008_robust_aidat_view_faiz_fix.sql
-- Description: Update aidat_detaylari view to respect faiz_yansitildi flag.
...
SELECT 
    ...
    -- 1. Tahakkuk (Accrual): expected = Base * Ratio + (Interest if reflected)
    ...
    -- 3. Faiz (Interest) - Respect faiz_yansitildi
    CASE WHEN a.faiz_yansitildi THEN COALESCE(a.gecikme_faizi, 0) ELSE 0 END as toplam_faiz,
    CASE WHEN a.faiz_yansitildi THEN COALESCE(a.gecikme_faizi, 0) ELSE 0 END as gecikme_faizi, -- Legacy alias
...
-- Recreate summary functions if they depend on this view
