...
router.post('/', validate({ body: blokSchema }), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
...
router.put('/:id', validate({ body: blokSchema.partial() }), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
